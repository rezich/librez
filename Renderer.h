...
inline LCDBitmap* new_bitmap(int width, int height, LCDColor bgcolor) {
    return pd->graphics->newBitmap(width, height, bgcolor);
}
...
void clip_set(Rect r) {
    pd->graphics->setClipRect(r.x, r.y, r.w, r.h);
#ifdef USING_CUSTOM_RENDERER
    clip_rect = r;
...
inline void _set_point(bool mask, int x, int y, LCDColor color) {
    ...
    assert(color != kColorClear);
    if (color == kColorXOR) color = (1 - ((row[x / 8] >> (7 - (x % 8))) & 1)) ? kColorWhite : kColorBlack;
    if (color > kColorXOR) {
        LCDPattern* pattern = (LCDPattern*)color;
        if (!((*pattern)[8 + (y % 8)] & (1 << (x % 8)))) return;
        color = (!((*pattern)[(y % 8)] & (1 << (x % 8)))) ? kColorBlack : kColorWhite;
    }
    if (color == kColorBlack) { *byte = ~(~*byte | (0x80 >> (x % 8))); return; }
    if (color == kColorWhite) { *byte = *byte | (0x80 >> (x % 8));  return; }
}

void _draw_point(int x, int y, LCDColor color) {
...
void _draw_hline(int x, int y, int width, LCDColor color) {
    if (color == kColorClear) return;
    for (int i = x; i <= x + width; ++i) _draw_point(i, y, color); //TODO: optimize
}

void _draw_vline(int x, int y, int height, LCDColor color) {
    if (color == kColorClear) return;
    for (int i = y; i <= y + height; ++i) _draw_point(x, i, color); //TODO: optimize(?)
}

void _draw_line(Point a, Point b, LCDColor color) {
    if (color == kColorClear) return;
